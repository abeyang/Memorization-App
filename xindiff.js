/*
 ...
 This software is published under the terms of GPL.
 See the GNU General Public License for more details at
 http://www.gnu.org/copyleft/gpl.html.
*/

function lowerBound(array, begin, end, value) {
    if (begin > end) {
        return null;
    }
    ...
    if (begin == end) {
        return (array[begin] >= value) ? begin : null;
    }

    var middle = Math.floor(begin + (end - begin) / 2);
    if (array[middle] == value) {
        return middle;
    }

    if (array[middle] < value) {
        return lowerBound(array, middle + 1, end, value);
    }

    var position = lowerBound(array, begin, middle - 1, value);
    return (position != null) ? position : middle;
}

function match(element, index, begin, end) {
    element = '_' + element;
    if (index.hasOwnProperty(element)) {
        var locations = index[element];
        var position = lowerBound(locations, 0, locations.length - 1, begin);
...
function DiffLinks() {
    ...
    this.pack = function(tailLinks, links) {
        for (var p = this; p != null; p = p.next) {
            links.pushBackLinks(p);
        }
        links.pushBackLinks(tailLinks);
        links.count += this.count + tailLinks.count;
        return links;
    }
    ...
    this.serialize = function(diff, output) {
		...
		for (var id in this.ranges) {
			if (id.length > 2) continue;	// fix for prototype
            ...
            for (var j = position1; j <= i.second; ++j) {
                ...
                var overlap = match(sequence1[j], diff.index[0], position0, end0);
                if (position0 < overlap) {
                    if (last1 < j) {
                        output.onSame(last1, j - 1,
                            position0 - j + last1, position0 - 1);
                        last1 = j;
                    }
            ...
            if (last1 < position1) {
                output.onSame(last1, position1 - 1,
                    position0 - position1 + last1, position0 - 1);
            }
        }
...
function DiffEngine() {
    this.doDiff = function() {
        ...
        for (;;) {
            ...
            if (match(element0, this.index[1], begin1, end1) == null) {
                --end0;
                continue;
            }

            if (match(element1, this.index[0], begin0, end0) == null) {
                --end1;
                continue;
            }
        ...
        var links = new DiffLinks();
        return this.diff(begin0, end0, begin1, end1, {}, -1).pack(tailLinks, links);
    }
    ...
    this.diff = function(begin0, end0, begin1, end1, cache, minLinks) {
        ...
        var size0 = end0 - begin0 + 1;
        var size1 = end1 - begin1 + 1;
        if ((size0 < minLinks) || (size1 < minLinks)) {
            return links;
        }
        ...
        for (;;) {
            ...
            overlap1 = match(element0, this.index[1], begin1, end1);
            if (overlap1 == null) {
                ++begin0;
                --size0;
                if (size0 < minLinks) {
            ...
            overlap0 = match(element1, this.index[0], begin0, end0);
            if (overlap0 == null) {
                ++begin1;
                --size1;
                if (size1 < minLinks) {
        ...
        if ((overlap0 - begin0) <= (overlap1 - begin1)) {
            var links0 = this.diff(overlap0, end0, begin1, end1, cache, minLinks);
            if (links0.count > (end1 - overlap1)) {
                linksx = links0;
            }
            else {
                var links1 = this.diff(begin0, end0, overlap1, end1, cache, Math.max(minLinks, links0.count));
                linksx = (links0.count >= links1.count) ? links0 : links1;
            }
        }
        else {
            var links1 = this.diff(begin0, end0, overlap1, end1, cache, minLinks);
            if (links1.count > (end0 - overlap0)) {
                linksx = links1;
            }
            else {
                var links0 = this.diff(overlap0, end0, begin1, end1, cache, Math.max(minLinks, links1.count));
                linksx = (links1.count >= links0.count) ? links1 : links0;
            }
        }

        minLinks = Math.max(minLinks, linksx.count);
        var count0 = Math.min(end0-begin0, end1-begin1) + 1 - minLinks;
        if (count0 > 1) {
            var count1 = Math.min(overlap0-begin0, overlap1-begin1) + 1;
            for (; (count0 > 1) && (count1 > 1); --count0, --count1) {
                ++begin0;
                ++begin1;
                var element0 = this.sequences[0][begin0];
                var element1 = this.sequences[1][begin1];
                if ((element0 == element1) ||
                    (match(element0, this.index[1], begin1, overlap1) != null) ||
                    (match(element1, this.index[0], begin0, overlap0) != null)) {
                    var links1 = this.diff(begin0, end0, begin1, end1, cache, minLinks);
    ...
    this.assign = function(sequence, id) {
        ...
        for (var i = 0; i < length; ++i) {
            var element = '_' + sequence[i];
            if (this.index[id].hasOwnProperty(element)) {
                this.index[id][element].push(i);
            }
            else {
                this.index[id][element] = [i];
            }
        }
    }
...
function DiffOutput() {
    ...
    this.onBegin = function() {
        ...
        this.lastDeleted = (0 in this.symbols[1]) ? this.symbols[1][0] : null;
        this.lastInserted = (0 in this.symbols[0]) ? this.symbols[0][0] : null;
    }
    ...
    this.onSame = function(beginLeft, endLeft, beginRight, endRight) {
        ...
        for (;;) {
            ...
            ++i; ++j;
            if (i > endLeft) {
                this.lastDeleted = (i in this.symbols[1]) ? this.symbols[1][i] : null;
                this.lastInserted = (j in this.symbols[0]) ? this.symbols[0][j] : null;
                break;
            }
            var left = (i in this.symbols[1]);
            var right = (j in this.symbols[0]);
            if (left && right) {
                left = this.symbols[1][i];
                right = this.symbols[0][j];
                if (left == right) {
                ...
                this.html.push('<del>');
                this.escapeText(this.symbols[1][i]);
                this.html.push('</del>');
            }
            else if (right) {
                this.html.push('<ins>');
                this.escapeText(this.symbols[0][j]);
                this.html.push('</ins>');
            }
        }
    }
    ...
    this.onInserted = function(begin, end) {
        ...
        ++end;
        if (end in this.symbols[0]) {
            this.lastInserted = this.symbols[0][end];
        }
    }
    ...
    this.onDeleted = function(begin, end) {
        ...
        ++end;
        if (end in this.symbols[1]) {
            this.lastDeleted = this.symbols[1][end];
        }
    }
    ...
    this.dumpSequence = function(begin, end, id) {
        for (var i = begin;;) {
            ...
            ++i;
            if (i in this.symbols[id]) {
                this.escapeText(this.symbols[id][i]);
            }
        }
    }
